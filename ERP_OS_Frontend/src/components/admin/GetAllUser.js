...
import { AdminloadAllStaff } from "../../redux/actions/user/getStaffAction";
...
function CustomTable({ list }) {
  ...
  const onChange = (value) => {
    setStatus(value);
    dispatch(AdminloadAllStaff({ status: value }));
  };
...
const GetAllCust = (props) => {
  ...
  useEffect(() => {
    dispatch(AdminloadAllStaff({ status: "true" }));
  }, []);
