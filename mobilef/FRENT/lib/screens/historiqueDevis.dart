...
class _HistoriqueDevisScreenState extends State<HistoriqueDevisScreen> {
  List<VenteBonCommande> _allDevis = [];
  List<VenteBonCommande> _filteredDevis = [];
  ...
  void _filterDevis() {
    ...
    setState(() {
      _filteredDevis =
          _allDevis.where((devis) {
            final matchesSearch =
                devis.reference.toLowerCase().contains(query) ||
  ...
  Widget _buildDevisCard(VenteBonCommande devis, int index) {
    return Card(
              ...
              children: [
                ...
                /*  if (devis['type'] == 'formulaire')
                  IconButton(
                    ...
                    onPressed: () => _editDevis(devis.toJson()),
                  ),
                  */
                IconButton(
    ...
    return pw.Container(
      ...
      child: pw.Column(
        children: [
          ...
          pw.Text('Référence: ${devis['reference']}', style: styles['bold']),
          ...
          if (devis['date'] != null)
            pw.Padding(
              ...
              child: pw.Text('Date: ${devis['date']}', style: styles['normal']),
            ),
        ],
      ),
    );
...
class EditFormulaireDevisScreen extends StatelessWidget {
  ...
  const EditFormulaireDevisScreen({Key? key, required this.devis})
    : super(key: key);

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Modifier devis formulaire')),
      body: Center(child: Text('Écran de modification pour devis formulaire')),
    );
  }
}
...
class EditPhotoDevisScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Modifier devis photo')),
      body: Center(child: Text('Écran de modification pour devis photo')),
    );
  }
}
